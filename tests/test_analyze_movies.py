...
def test_main_outputs_top_rated_correctly(tmp_path, capsys):
    """The Dark Knight (9.0) should appear before Inception (8.8) in output"""
    db_path = str(tmp_path / "movies.db")
    create_temp_db(db_path)
...
def test_main_outputs_average_rating(tmp_path, capsys):
    """Average rating should be printed and be a valid number"""
    db_path = str(tmp_path / "movies.db")
    create_temp_db(db_path)
    ...
    # The average rating is the last printed value
...
def test_main_outputs_most_popular(tmp_path, capsys):
    """The Dark Knight (80.1) should be the most popular and appear first"""
    db_path = str(tmp_path / "movies.db")
    create_temp_db(db_path)
...
def test_main_raises_on_missing_db(tmp_path):
    """main() should raise an exception if the DB does not exist"""
